...
class Soundcloud(object):
    ...
    def users(self, scid=None, filter_=None):
        """A SoundCloud user
        """
        return _api.Users(self.authinfo, scid=scid, filter=filter_)

    def tracks(self, scid=None, filter_=None):
        """A SoundCloud track
        """
        return _api.Tracks(self.authinfo, scid=scid, filter=filter_)

    def playlists(self, scid=None, filter_=None):
        """A SoundCloud Set is internally called playlists due to some naming
        restrictions.
        """
        return _api.Playlists(self.authinfo, scid=scid, filter=filter_)
    ...
    def comments(self, scid):
        """Comments can be made on tracks by any user who has access to a
        track, except for non commentable tracks. As you see in the SoundCloud
        player comments can also be associated with a specific timestamp in a
        track.
        """
